...
public class ObsEndLichenBlock extends Block implements Fertilizable {
    public ObsEndLichenBlock(AbstractBlock.Settings settings) {
        super(settings);
    }
    ...
    public void grow(ServerWorld world, Random random, BlockPos pos, BlockState state) {
        ...
        BlockPos blockPos = pos.up();
        if (blockState.isOf(ModBlocks.CHORUS_LICHEN)) {
        ...
        } else if (blockState.isOf(ModBlocks.CHORUS_LICHEN)) {
            NetherForestVegetationFeature.generate(world, random, blockPos, ConfiguredFeatures.Configs.WARPED_ROOTS_CONFIG, 3, 1);
            NetherForestVegetationFeature.generate(world, random, blockPos, ConfiguredFeatures.Configs.NETHER_SPROUTS_CONFIG, 3, 1);
